import streamlit as st
import pandas as pd
import folium
from streamlit_folium import st_folium
import requests
import datetime
import json
import os
import time
from streamlit_autorefresh import st_autorefresh

# ==========================================
# CONFIGURATION
# ==========================================
st.set_page_config(page_title="Incendies & Forêts à Risque", page_icon="🌲", layout="wide")

FRANCE_BBOX = {"lat_min": 41.3, "lat_max": 51.1, "lon_min": -5.2, "lon_max": 9.6}

CACHE_DIR = "cache_data"
os.makedirs(CACHE_DIR, exist_ok=True)
FIRES_JSON_PATH = os.path.join(CACHE_DIR, "fires_data.json")
FORESTS_GEOJSON_PATH = os.path.join(CACHE_DIR, "forests_data.geojson")
WEATHER_JSON_PATH = os.path.join(CACHE_DIR, "weather_data.json")

def is_cache_valid(filepath, max_age_seconds):
    if os.path.exists(filepath):
        return (time.time() - os.path.getmtime(filepath)) < max_age_seconds
    return False

# ==========================================
# 1. INCENDIES ACTIFS (NASA FIRMS)
# ==========================================
@st.cache_data(ttl=1800)
def fetch_fires_data():
    if is_cache_valid(FIRES_JSON_PATH, 1800):
        try:
            df = pd.read_json(FIRES_JSON_PATH)
            df['datetime'] = pd.to_datetime(df['datetime_str'])
            return df
        except Exception:
            pass

    urls = [
        "https://firms.modaps.eosdis.nasa.gov/data/active_fire/suomi-npp-viirs-c2/csv/SUOMI_NPP_VIIRS_C2_Europe_7d.csv",
        "https://firms.modaps.eosdis.nasa.gov/data/active_fire/modis-c6.1/csv/MODIS_C6_1_Europe_7d.csv"
    ]
    
    dfs = []
    for url in urls:
        try:
            df_raw = pd.read_csv(url)
            df_fr = df_raw[
                (df_raw['latitude'] >= FRANCE_BBOX["lat_min"]) & 
                (df_raw['latitude'] <= FRANCE_BBOX["lat_max"]) &
                (df_raw['longitude'] >= FRANCE_BBOX["lon_min"]) & 
                (df_raw['longitude'] <= FRANCE_BBOX["lon_max"])
            ].copy()
            if not df_fr.empty:
                dfs.append(df_fr)
        except Exception:
            continue

    if not dfs:
        return pd.DataFrame()

    data = pd.concat(dfs, ignore_index=True)
    data['acq_time'] = data['acq_time'].astype(str).str.zfill(4)
    data['datetime_str'] = data['acq_date'] + ' ' + data['acq_time'].str[:2] + ':' + data['acq_time'].str[2:] + ':00'
    data['datetime'] = pd.to_datetime(data['datetime_str'])

    # Récupérer uniquement les incendies très récents (36h)
    cutoff = datetime.datetime.utcnow() - datetime.timedelta(hours=36)
    data_recent = data[data['datetime'] >= cutoff]

    columns_to_keep = ['latitude', 'longitude', 'datetime_str', 'confidence']
    data_clean = data_recent[columns_to_keep].copy()

    try:
        data_clean.to_json(FIRES_JSON_PATH, orient="records")
    except Exception:
        pass

    data_clean['datetime'] = pd.to_datetime(data_clean['datetime_str'])
    return data_clean

# ==========================================
# 2. MÉTÉO DE SÉCHERESSE (OPEN-METEO)
# ==========================================
@st.cache_data(ttl=3600)
def fetch_weather_grid():
    if is_cache_valid(WEATHER_JSON_PATH, 3600):
        try:
            with open(WEATHER_JSON_PATH, 'r') as f:
                return json.load(f)
        except Exception:
            pass

    # Grille stratégique couvrant les grands massifs français
    points = [
        (43.5, 6.0), (43.8, 5.0), (44.5, -0.5), (44.2, 3.5), 
        (43.0, 2.5), (48.4, 2.6), (48.5, 7.2), (47.3, 3.8)
    ]
    
    lat_str = ",".join([str(p[0]) for p in points])
    lon_str = ",".join([str(p[1]) for p in points])
    
    url = f"https://api.open-meteo.com/v1/forecast?latitude={lat_str}&longitude={lon_str}&current=temperature_2m,relative_humidity_2m,wind_speed_10m"
    
    weather_dict = {}
    try:
        res = requests.get(url, timeout=10).json()
        for i, loc in enumerate(res):
            cur = loc.get("current", {})
            weather_dict[f"{points[i][0]},{points[i][1]}"] = {
                "temp": cur.get("temperature_2m", 20),
                "humidity": cur.get("relative_humidity_2m", 50),
                "wind": cur.get("wind_speed_10m", 10)
            }
        with open(WEATHER_JSON_PATH, 'w') as f:
            json.dump(weather_dict, f)
    except Exception:
        pass

    return weather_dict

# ==========================================
# 3. CONTOURS DES FORÊTS (OVERPASS API / OSM)
# ==========================================
@st.cache_data(ttl=86400) # Cache de 24h pour les massifs forestiers
def fetch_forest_polygons():
    if is_cache_valid(FORESTS_GEOJSON_PATH, 86400):
        try:
            with open(FORESTS_GEOJSON_PATH, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception:
            pass

    # Requête Overpass ciblée sur les principaux massifs forestiers et forêts de pins
    overpass_query = """
    [out:json][timeout:25];
    (
      way["landuse"="forest"](43.0,-1.5,45.0,0.5);   // Landes de Gascogne
      way["landuse"="forest"](43.0,5.0,44.5,7.5);    // Var / Maures / Estérel
      way["landuse"="forest"](43.2,3.0,44.2,4.8);    // Cévennes / Gard / Hérault
      way["landuse"="forest"](48.2,2.3,48.6,2.9);    // Forêt de Fontainebleau
      way["landuse"="forest"](48.0,6.5,49.0,7.8);    // Vosges
    );
    out body;
    >;
    out skel qt;
    """
    
    url = "https://overpass-api.de/api/interpreter"
    try:
        response = requests.post(url, data={'data': overpass_query}, timeout=30)
        data = response.json()
        
        nodes = {node['id']: (node['lon'], node['lat']) for node in data.get('elements', []) if node['type'] == 'node'}
        features = []
        
        for elem in data.get('elements', []):
            if elem['type'] == 'way' and 'nodes' in elem:
                coords = [nodes[nid] for nid in elem['nodes'] if nid in nodes]
                if len(coords) >= 4 and coords[0] == coords[-1]:
                    tags = elem.get('tags', {})
                    leaf_type = tags.get('leaf_type', 'unknown')
                    name = tags.get('name', 'Zone Forestière')
                    
                    # Détection des forêts de pins / résineux
                    is_pine = (leaf_type in ['needleleaved', 'mixed']) or ('pin' in name.lower())
                    
                    feature = {
                        "type": "Feature",
                        "geometry": {
                            "type": "Polygon",
                            "coordinates": [coords]
                        },
                        "properties": {
                            "name": name,
                            "leaf_type": leaf_type,
                            "is_pine": is_pine
                        }
                    }
                    features.append(feature)
                    
        geojson = {"type": "FeatureCollection", "features": features}
        
        with open(FORESTS_GEOJSON_PATH, 'w', encoding='utf-8') as f:
            json.dump(geojson, f)
            
        return geojson
    except Exception:
        return {"type": "FeatureCollection", "features": []}

# ==========================================
# UTILITAIRE : ANALYSE DU RISQUE PAR FORÊT
# ==========================================
def analyze_forest_risk(feature, df_fires, weather_data):
    coords = feature['geometry']['coordinates'][0]
    lons = [c[0] for c in coords]
    lats = [c[1] for c in coords]
    
    min_lon, max_lon = min(lons), max(lons)
    min_lat, max_lat = min(lats), max(lats)
    center_lat, center_lon = (min_lat + max_lat) / 2, (min_lon + max_lon) / 2

    # 1. Vérification si un incendie actif est dans ou très proche du polygone
    has_fire = False
    if not df_fires.empty:
        fires_near = df_fires[
            (df_fires['latitude'] >= min_lat - 0.03) & (df_fires['latitude'] <= max_lat + 0.03) &
            (df_fires['longitude'] >= min_lon - 0.03) & (df_fires['longitude'] <= max_lon + 0.03)
        ]
        has_fire = not fires_near.empty

    if has_fire:
        return {
            "status": "INCENDIE ACTIF",
            "color": "#D32F2F",       # Rouge vif
            "fillColor": "#FF1744",
            "opacity": 0.85,
            "risk_label": "🔥 Incendie en cours détecté par satellite"
        }

    # 2. Évaluation de la sécheresse météo à proximité
    is_pine = feature['properties'].get('is_pine', False)
    
    # Recherche de la station météo la plus proche dans la grille
    closest_weather = {"temp": 22, "humidity": 45, "wind": 15}
    min_dist = 999
    for key, w_info in weather_data.items():
        w_lat, w_lon = map(float, key.split(','))
        dist = ((w_lat - center_lat)**2 + (w_lon - center_lon)**2)**0.5
        if dist < min_dist:
            min_dist = dist
            closest_weather = w_info

    temp = closest_weather['temp']
    hum = closest_weather['humidity']
    wind = closest_weather['wind']

    # Conditions de fort risque : Climat sec (Temp > 24°C, Humidité < 40%) + Pins/Résineux
    is_very_dry = (temp >= 24 and hum <= 42) or (temp >= 28) or (hum <= 30)
    
    if is_very_dry and is_pine:
        return {
            "status": "FORÊT TRÈS SÈCHE (PINS) - RISQUE EXTRÊME",
            "color": "#E65100",       # Orange foncé / Rougeâtre
            "fillColor": "#FF5722",
            "opacity": 0.65,
            "risk_label": f"⚠️ Très sèche (Temp: {temp}°C, Humidité: {hum}%, Pins/Résineux)"
        }
    elif is_very_dry or (is_pine and temp > 22):
        return {
            "status": "RISQUE ÉLEVÉ DE PROPAGATION",
            "color": "#F57C00",       # Orange
            "fillColor": "#FF9800",
            "opacity": 0.55,
            "risk_label": f"⚡ Risque élevé (Temp: {temp}°C, Humidité: {hum}%)"
        }
    else:
        return {
            "status": "RISQUE FAIBLE / MODÉRÉ",
            "color": "#2E7D32",       # Vert forêt
            "fillColor": "#4CAF50",
            "opacity": 0.35,
            "risk_label": f"🌲 Conditions normales (Temp: {temp}°C, Humidité: {hum}%)"
        }

# ==========================================
# APPLICATION PRINCIPALE
# ==========================================
def main():
    st.title("🌲 Zones Forestières & Incendies en France")
    
    st_autorefresh(interval=5 * 60 * 1000, key="datarefresh")
    
    with st.spinner("Chargement des contours de forêts et des données météo/satellites..."):
        df_fires = fetch_fires_data()
        weather_data = fetch_weather_grid()
        forests_geojson = fetch_forest_polygons()

    # Métriques directes en haut de la carte
    col1, col2, col3 = st.columns(3)
    col1.metric("🔥 Incendies actifs (36h)", len(df_fires) if not df_fires.empty else 0)
    col2.metric("🌲 Massifs forestiers analysés", len(forests_geojson.get('features', [])))
    col3.metric("⏱️ Actualisation", datetime.datetime.now().strftime("%H:%M"))

    # Carte Folium unique
    m = folium.Map(location=[45.5, 2.5], zoom_start=6, tiles="CartoDB dark_matter")

    # Application du style GeoJSON pour chaque forêt
    if forests_geojson.get('features'):
        for feature in forests_geojson['features']:
            risk = analyze_forest_risk(feature, df_fires, weather_data)
            
            name = feature['properties'].get('name', 'Massif forestier')
            leaf_type = feature['properties'].get('leaf_type', 'non spécifié')
            
            popup_html = f"""
            <div style="font-family: Arial; font-size: 13px; min-width: 200px;">
                <b style="font-size: 14px;">{name}</b><br><br>
                <b>État :</b> <span style="color:{risk['color']};"><b>{risk['status']}</b></span><br>
                <b>Détail :</b> {risk['risk_label']}<br>
                <b>Type de végétation :</b> {leaf_type}
            </div>
            """

            folium.GeoJson(
                feature,
                style_function=lambda x, r=risk: {
                    'fillColor': r['fillColor'],
                    'color': r['color'],
                    'weight': 2,
                    'fillOpacity': r['opacity']
                },
                tooltip=f"{name} - {risk['status']}",
                popup=folium.Popup(popup_html, max_width=300)
            ).add_to(m)

    # Légende explicative directe
    st.markdown("""
    **Légende de la carte :**  
    🔴 **Rouge** : Forêt avec un foyer d'incendie actif détecté  |  
    🟠 **Orange / Rouge vif** : Forêt très sèche & résineuse à risque extrême de propagation  |  
    🟢 **Vert** : Zone forestière à risque normal / faible
    """)

    # Affichage de la carte plein écran
    st_folium(m, width="100%", height=720, returned_objects=[])

if __name__ == "__main__":
    main()